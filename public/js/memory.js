...
export async function initMemory(container, { project, api }) {
  ...
  container.innerHTML = `
    <div class="memory-grid">
      ...
      <details class="panel settings-wide memory-exploration">
        <summary><span><p class="eyebrow">Explore the graph</p><strong>Graph overview, path, and neighborhood</strong></span><span class="badge">3 tools</span></summary>
        <div class="memory-exploration-body">
      <article class="memory-tool"><div class="panel-header"><div><p class="eyebrow">Structural analysis</p><h2>Graph overview</h2></div><div class="button-row"><span class="badge">${status.counts.nodes || 0} nodes</span><button class="button" id="loadGraphOverview">Load overview</button></div></div><div class="panel-body"><div id="memoryOverviewResults" class="memory-results"><p class="settings-copy">God nodes rank structural centrality; communities group code that imports or references each other. Both load the whole graph, up to 5,000 nodes.</p></div></div></article>
      <article class="memory-tool"><div class="panel-header"><div><p class="eyebrow">Recorded connections</p><h2>Path</h2></div></div><div class="panel-body"><form class="context-compiler-form" id="memoryPathForm"><div class="field"><label for="memoryPathFrom">From</label><input id="memoryPathFrom" autocomplete="off" placeholder="memory-service" maxlength="500"></div><div class="field"><label for="memoryPathTo">To</label><input id="memoryPathTo" autocomplete="off" placeholder="timeline-service" maxlength="500"></div><button class="button" type="submit">Find path</button></form><div id="memoryPathResults" class="memory-results"><p class="settings-copy">Each endpoint resolves through search; the hop sequence follows persisted edges only.</p></div></div></article>
      <article class="memory-tool"><div class="panel-header"><div><p class="eyebrow">Focused graph explorer</p><h2>Neighborhood</h2></div></div><div class="panel-body"><div class="memory-graph-controls"><label>Depth <select id="memoryGraphDepth"><option value="1">1</option><option value="2" selected>2</option><option value="3">3</option><option value="4">4</option></select></label>${['CONTAINS', 'IMPORTS', 'REFERENCES'].map((type) => `<label><input type="checkbox" name="memoryEdgeType" value="${type}" checked> ${type}</label>`).join('')}</div><div id="memoryGraphResults" class="memory-results">${emptyState('◎', 'Select a memory result', 'The explorer loads only the selected node neighborhood and keeps a list fallback.')}</div></div></article>
        </div>
      </details>
    </div>`;
  ...
  container.querySelector('#loadGraphOverview').addEventListener('click', async (event) => {
    ...
    try {
      ...
      // The service caps whole-graph analysis; a partial answer must say so
      // rather than read as a complete picture of the repository.
      const truncated = central.truncated || grouped.truncated
        ? `<p class="field-hint memory-truncated">Analysis covered ${central.analyzedNodes} of ${central.totalNodes} indexed nodes. Nodes beyond the cap were not ranked or grouped.</p>`
        : '';
      target.innerHTML = `${truncated}${godNodes(central)}${communities(grouped)}`;
      bindNodes(target);
    } catch (error) {
      showToast(error.message, 'error');
    } finally {
